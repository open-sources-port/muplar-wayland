...
impl CompositorState {
    ...
    pub fn send_toplevel_configure(
        ...
        crate::wlog!(
            ...
            "send_toplevel_configure: client={:?} tl_id={} size={}x{} serial={}",
            client_id,
            toplevel_id,
            width,
            height,
            ...
            .get_mut(&(client_id.clone(), toplevel_id))
        {
            ...
            let (final_w, final_h) =
                if toplevel_data.pending_fullscreen || (width == 0 && height == 0) {
                    (width, height)
                } else {
            ...
            if let Some(resource) = &toplevel_data.resource {
                ...
                if toplevel_data.pending_fullscreen {
                    states.extend_from_slice(
                        &(wayland_protocols::xdg::shell::server::xdg_toplevel::State::Fullscreen
                            as u32)
                            .to_ne_bytes(),
                    );
                }
                ...
                crate::wlog!(
                    crate::util::logging::COMPOSITOR,
                    "Configuring xdg_toplevel {} with states: {:?}, size={}x{}",
                    toplevel_id,
                    states,
                    final_w,
                    final_h
                ...
                crate::wlog!(
                    crate::util::logging::COMPOSITOR,
                    "send_toplevel_configure: No resource for tl_id={}",
                    toplevel_id
                );
            }
        } else {
        ...
        if let Some((toplevel, xdg_surface_id, states, final_w, final_h)) = to_send {
            ...
            if let Some(surface_data) = self.xdg.surfaces.get_mut(&(client_id, xdg_surface_id)) {
                ...
                if let Some(resource) = &surface_data.resource {
                    crate::wlog!(
                        crate::util::logging::COMPOSITOR,
                        "Actually sending xdg_surface.configure(serial={}) to xdg_surface_id={}",
                        serial,
                        xdg_surface_id
                    );
                    ...
                    crate::wlog!(
                        crate::util::logging::COMPOSITOR,
                        "No resource for xdg_surface_id={} when sending configure",
                        xdg_surface_id
                    );
                }
            } else {
                crate::wlog!(
                    crate::util::logging::COMPOSITOR,
                    "xdg_surface_id={} NOT FOUND in xdg.surfaces",
                    xdg_surface_id
                );
            }
        }
    ...
    pub fn build_scene(&mut self) {
        ...
        for window_id in self.windows.keys().copied().collect::<Vec<_>>() {
            if let Some(window) = self.get_window(window_id) {
                ...
                let geom_offset = geom_by_surface
