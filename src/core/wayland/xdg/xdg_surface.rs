...
use crate::core::state::{CompositorState, XdgPopupData, XdgToplevelData};
        ...
        match request {
            xdg_surface::Request::GetToplevel { id } => {
                crate::wlog!(
                    ...
                    "xdg_surface.get_toplevel for surface {}",
                    xdg_surface_id
                );
                if let Some(data) = data {
                    ...
                    state.xdg.toplevels.insert(
                        (client_id.clone(), toplevel.id().protocol_id()),
                        toplevel_data,
                    );
                    ...
                    if let Some(surface) = state.get_surface(data.surface_id) {
                        ...
                        if let Err(e) =
                            surface.set_role(crate::core::surface::SurfaceRole::Toplevel)
                        {
                            tracing::error!(
                                "Failed to set role for surface {}: {}",
                                data.surface_id,
                                e
                            );
                        }
            ...
            xdg_surface::Request::GetPopup {
                ...
                if let Some(data) = data {
                    ...
                    let output_rect =
                        crate::util::geometry::Rect::new(ox, oy, initial_width, initial_height);
                    ...
                    let popup: wayland_protocols::xdg::shell::server::xdg_popup::XdgPopup =
                        data_init.init(id, window_id);
                    ...
                    state
                        .xdg
                        .popups
                        .insert((client_id.clone(), popup.id().protocol_id()), popup_data);
                    ...
                    tracing::debug!(
                        "Created xdg_popup for surface {}, window_id={}",
                        wl_surface_id,
                        window_id
                    );
                    ...
                    if let Some(window_id) = data.window_id {
                        if let Some(window) = state.get_window(window_id) {
                            ...
                            for ((cid, tl_proto_id), tl_data) in state.xdg.toplevels.iter() {
                                if *cid == client_id
                                    && tl_data.xdg_surface_id == xdg_surface_id
                                    && tl_data.pending_serial == serial
                                {
